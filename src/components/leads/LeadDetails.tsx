...
export default function LeadDetailsModal({ open, setOpen, lead }: LeadDetailsModalProps) {
    return (
        lead && (
            <Modal open={open} setOpen={setOpen} header={<Header name={lead.name} />} body={<Body name={lead.name} />} footer={<Footer />} />
        )
    )
};
...
const Body = ({ name }: { name: string }) => {
    ...
    return (
        <div className="w-full">
            ...
            <div className="rounded-lg p-2 bg-gradient-to-r from-blue-50 to-purple-50 mt-4 flex flex-wrap gap-2">
                <div className="flex items-center gap-x-2 text-gray-700 bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">
                    <SparklesIcon className="size-4" />
                    <p className="mt-px text-transparent text-xs">{firstName} may be interested in upgrading espresso machines for her in-store coffee shops.</p>
                </div>
